...
def generate_dynamic_key(batch_size, key_size_per_step, dynamic_steps, total_steps=28, correct_key=True,
    ...
    if correct_key:
        ...
        # ######
        core_seed = sample_seed if sample_seed is not None else base_seed
        for step in range(dynamic_steps):
            # Use step + core seed as deterministic seed to generate different random patterns per step
            step_rng = RandomState(core_seed + step)
...
def collect_states(X_data, esn_cell, use_correct_key=True, return_all_timesteps=False, custom_key=None,
    ...
    for i in range(n_samples):  #
        ...
        if custom_key is not None:
            ...
            key_inputs = generate_dynamic_key(
                1, key_size_per_step, dynamic_steps, n_steps, use_correct_key,
                rng, base_seed, sample_seed=sample_seed  # 传入样本专属seed
            ).squeeze(0)
