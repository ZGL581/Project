...
# Set font to support Western characters (remove Chinese font dependencies)
...
class EchoStateRNNCell:
    def __init__(self, num_units, num_inputs, key_size=32, activation=np.tanh, decay=0.1,
        ...
        # Partition reservoir nodes into input-dedicated and output-dedicated subsets
        ...
        # 1.1 Sparse input-reservoir connections
...
# 3. Generate dynamic keys
def generate_dynamic_key(key_size, timestep, seed=789):
    """Generate independent 32-bit dynamic key for a single timestep (binaryized)"""
    # Use different sub-seeds for each timestep to ensure reproducibility of the same key sequence
    rng = np.random.RandomState(seed + timestep)
    dynamic_key = rng.rand(key_size)
    dynamic_key = (dynamic_key > 0.5).astype(np.float32).reshape(1, key_size)
    return dynamic_key


def generate_dynamic_key_series(n_timesteps, key_size, seed=789):
    """Generate dynamic key series: independent 32-bit key for each timestep (total length 32*n_timesteps bits)"""
    key_series = [generate_dynamic_key(key_size, t, seed) for t in range(n_timesteps)]
    return key_series
...
def collect_states_with_timesteps(X_data, esn_cell, key_series):
    ...
    for i in range(n_samples):
        ...
        for t in range(n_timesteps):
            ...
            key_t = key_series[t]  # Use independent dynamic key for each timestep
...
def visualize_last_timestep_states(states, title, save_path, key_type, n_timesteps, output_dir):
    """Visualize reservoir states at the last timestep (3D PCA) and save trajectory data"""
    n_samples, _, n_neurons = states.shape
    last_step = n_timesteps - 1
    last_step_states = states[:, last_step, :]  # Get last timestep for all samples

    # PCA dimensionality reduction to 3D
    pca = PCA(n_components=3, random_state=42)
    states_3d = pca.fit_transform(last_step_states)
...
n_timesteps = 20  # Number of timesteps, corresponding to total dynamic key length 32*20 bits
...
n_neurons = 512
n_inputs = 1
key_size = 32  # Key length per timestep (bits)
n_outputs = 1
spectral_radius = 1.1
sparsity_res = 0.85
input_sparsity = 0.8
...
epochs = 400
lr = 0.001
output_dir = "esn_mackey_glass_dynamic_key"  # Modify output directory to indicate dynamic keys
...
# 10. Pre-generate dynamic key series
print("=" * 50)
print("Generating dynamic keys...")
correct_key_seed = 789
wrong_key_seed = 234

# Generate dynamic key series: independent 32-bit key per timestep, total length 32*20 bits
correct_dynamic_keys = generate_dynamic_key_series(n_timesteps, key_size, seed=correct_key_seed)
wrong_dynamic_keys = generate_dynamic_key_series(n_timesteps, key_size, seed=wrong_key_seed)

print(f"Correct dynamic key seed: {correct_key_seed} (total length {key_size}*{n_timesteps} = {key_size*n_timesteps} bits)")
print(f"Wrong dynamic key seed: {wrong_key_seed} (total length {key_size}*{n_timesteps} = {key_size*n_timesteps} bits)")
print(f"Dynamic key space size: 2^({key_size*n_timesteps}) = {2**(key_size*n_timesteps)}")

# 11. Collect training states
print("=" * 50)
print("Collecting training states (correct dynamic key only)...")
train_states = collect_states_with_timesteps(X_train, esn_cell, correct_dynamic_keys)
...
test_states_correct_viz = collect_states_with_timesteps(X_test[:viz_samples], esn_cell, correct_dynamic_keys)
test_states_wrong_viz = collect_states_with_timesteps(X_test[:viz_samples], esn_cell, wrong_dynamic_keys)

# Save complete states to CSV
save_states_to_csv(test_states_correct_viz, 'correct_dynamic_states.csv', output_dir, n_neurons)
save_states_to_csv(test_states_wrong_viz, 'wrong_dynamic_states.csv', output_dir, n_neurons)

# 13. 3D visualization and trajectory saving
print("=" * 50)
print("Generating 3D PCA visualizations...")

visualize_last_timestep_states(
    states=test_states_correct_viz,
    title=f'Last Timestep State Distribution of Mackey-Glass Reservoir with Correct Dynamic Key (Key Length {key_size}*{n_timesteps} bits)',
    save_path=os.path.join(output_dir, 'tates_correct_dynamic.png'),
    key_type="Correct_Dynamic_Key",
...
visualize_last_timestep_states(
    ...
    title=f'Last Timestep State Distribution of Mackey-Glass Reservoir with Wrong Dynamic Key (Key Length {key_size}*{n_timesteps} bits)',
    save_path=os.path.join(output_dir, 'states_wrong_dynamic.png'),
    key_type="Wrong_Dynamic_Key",
    n_timesteps=n_timesteps,
    output_dir=output_dir
)

# 14. Prepare training and test features (for predictor)
print("=" * 50)
...
# Test data
test_states_correct_full = collect_states_with_timesteps(X_test, esn_cell, correct_dynamic_keys)
test_states_wrong_full = collect_states_with_timesteps(X_test, esn_cell, wrong_dynamic_keys)
...
results = []
header = ['Epoch', 'Train MSE (Correct Dynamic Key)', 'Test MSE (Correct Dynamic Key)', 'Test MSE (Wrong Dynamic Key)', 'Performance_Gap']
results.append(header)
best_test_mse = float('inf')

print("=" * 50)
print("Starting training (correct dynamic key data only)...")
...
for epoch in range(epochs):
    ...
    if test_mse_correct < best_test_mse:
        ...
        torch.save(model.state_dict(), os.path.join(output_dir, 'best_model_dynamic.pth'))
...
with open(os.path.join(output_dir, 'dynamic_key_evaluation_results.csv'), 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerows(results)

# Load best model
model.load_state_dict(torch.load(os.path.join(output_dir, 'best_model_dynamic.pth')))
model.eval()
with torch.no_grad():
...
print(f"Correct dynamic key MSE: {mse_correct_final:.6f}")
print(f"Wrong dynamic key MSE: {mse_wrong_final:.6f}")
print(f"Performance gap: {mse_wrong_final / mse_correct_final:.2f}x")
print(f"Dynamic key space size: 2^({key_size*n_timesteps}) = {2**(key_size*n_timesteps)} (much larger than static key's 2^{key_size})")

# Save detailed prediction comparison
with open(os.path.join(output_dir, 'prediction_comparison_dynamic.csv'), 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['Timestep', 'True_Value', 'Correct_Dynamic_Key_Prediction', 'Wrong_Dynamic_Key_Prediction',
                     'Correct_Dynamic_Key_Absolute_Error', 'Wrong_Dynamic_Key_Absolute_Error'])
...
plt.plot(pred_correct_original[:200], label='Correct Dynamic Key', linestyle='--', color='blue')
plt.plot(pred_wrong_original[:200], label='Wrong Dynamic Key', linestyle=':', color='red', alpha=0.7)
plt.title('Sequence Prediction Comparison (First 200 Steps) - Dynamic Key')
plt.xlabel('Timestep')
...
plt.hist(errors_correct, bins=50, alpha=0.7, label='Correct Dynamic Key', color='blue', density=True)
plt.hist(errors_wrong, bins=50, alpha=0.7, label='Wrong Dynamic Key', color='red', density=True)
plt.xlabel('Absolute Error')
plt.ylabel('Density')
plt.title('Error Distribution - Dynamic Key')
plt.legend()
plt.yscale('log')

plt.subplot(1, 3, 3)
plt.scatter(y_test_original, pred_correct_original, alpha=0.5, s=10, label='Correct Dynamic Key', color='blue')
plt.scatter(y_test_original, pred_wrong_original, alpha=0.5, s=10, label='Wrong Dynamic Key', color='red')
plt.plot([y_test_original.min(), y_test_original.max()],
         [y_test_original.min(), y_test_original.max()], 'k--', label='Perfect Prediction')
plt.xlabel('True Value')
plt.ylabel('Predicted Value')
plt.title('Prediction vs True Value Scatter Plot - Dynamic Key')
plt.legend()

plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'dynamic_key_evaluation_summary.png'), dpi=300, bbox_inches='tight')
plt.close()

print(f"\nAll results saved to: {output_dir}")
print("Generated files:")
print("  - correct_dynamic_states.csv / wrong_dynamic_states.csv (complete reservoir states)")
print("  - trajectory_data.csv (PCA trajectory data)")
print("  - last_timestep_states_correct_dynamic.png / wrong_dynamic.png (3D PCA plots)")
print("  - dynamic_key_evaluation_results.csv (training logs)")
print("  - prediction_comparison_dynamic.csv (prediction comparison)")
print("  - dynamic_key_evaluation_summary.png (comprehensive visualization)")